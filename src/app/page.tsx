...
import { useState, useCallback, useRef, useEffect, DragEvent } from 'react'; // Added DragEvent
...
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, Legend as RechartsLegend, ResponsiveContainer, Scatter, ScatterChart } from 'recharts'; // Removed AreaChart, Area, Bar, Cell // Added Scatter, ScatterChart
...
import { generateModelCode } from '@/ai/flows/generate-model-code'; // Import the generateModelCode flow
import { describeModel } from '@/ai/flows/describe-model'; // Import the describeModel flow
...
export default function Home() {
   ...
   useEffect(() => {
    ...
    if (isLoading && startTime !== null) {
        intervalId = setInterval(() => {
            ...
            // Store current progress and time ONLY IF progress increased significantly
            const lastStoredProgress = progressRef.current.length > 0 ? progressRef.current[progressRef.current.length - 1].progress : 0;
            if (currentProgress > lastStoredProgress + 0.1) { // Update history if progress increases by at least 0.1%
                progressRef.current.push({ time: elapsed, progress: currentProgress });
                // Keep only recent history (e.g., last 10-20 seconds)
                const historyCutoff = elapsed - 20000; // Increase history window
            ...
            let remainingTimeMs = -1; // -1 indicates insufficient data or completion
             ...
             if (currentProgress > 0.1 && currentProgress < 99.9 && progressRef.current.length > 1) { // Use thresholds to avoid edge cases
                 // Use more robust calculation: linear regression or weighted average
                 let totalWeight = 0;
                 let weightedRateSum = 0;
                 const recentHistory = progressRef.current.slice(-10); // Use last 10 points for rate calculation

                 if (recentHistory.length > 1) {
                      for (let i = 1; i < recentHistory.length; i++) {
                          const p1 = recentHistory[i-1];
                          const p2 = recentHistory[i];
                          const progressDelta = p2.progress - p1.progress;
                          const timeDelta = p2.time - p1.time;

                          if (progressDelta > 0 && timeDelta > 100) { // Min time delta 100ms
                              const rate = progressDelta / timeDelta;
                              const weight = timeDelta; // Weight by time duration
                 ...
                 if (totalWeight > 0) {
                     const averageRate = weightedRateSum / totalWeight;
                     const remainingProgress = 100 - currentProgress;
                     remainingTimeMs = remainingProgress / averageRate;
                 } else {
                    // Fallback to simple start/end if weighted average fails
             ...
             // Smoothing the remaining time estimate only if it's positive
             if (remainingTimeMs > 0) {
                 const previousEstimate = remainingTimeRef.current;
                 const alpha = 0.2; // Smoothing factor (0 < alpha <= 1), maybe slightly higher
                 if (previousEstimate !== null) {
                     remainingTimeMs = alpha * remainingTimeMs + (1 - alpha) * previousEstimate;
                 }
                 // Clamp the estimate to a reasonable maximum (e.g., 1 hour)
                 remainingTimeMs = Math.min(remainingTimeMs, 3600000);
                 remainingTimeRef.current = remainingTimeMs; // Update ref only with valid positive estimate
             } else {
                  // Don't update the ref if calculation is invalid, keep the previous estimate
                  remainingTimeMs = remainingTimeRef.current ?? -1; // Use previous if current is invalid
             }


            if (remainingTimeMs > 0) {
                const remainingSeconds = Math.max(1, Math.round(remainingTimeMs / 1000)); // Show at least 1s
                ...
                } else { // Should not happen due to Math.max(1,...)
                   timeString = 'Finishing up...';
                }
                setRemainingTimeText(timeString);

            } else if (currentProgress === 0 && isLoading) {
                ...
                // If calculation failed or progress is stuck but not complete
                 ...
                 setRemainingTimeText(''); // Clear if stopped/not started and not complete
            }
  ...
  return (
    <div ref={reportRef} className="flex flex-col items-center justify-start min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-blue-300 via-blue-400 to-teal-500 dark:from-blue-800 dark:via-blue-900 dark:to-teal-900 text-foreground">
      {/* Render SVG Gradient Definitions */}
      {renderGradientDefs(chartConfig, parameters)}


      {/* Header Section */}
      <header className="w-full max-w-5xl mb-8 text-center text-white shadow-lg p-4 rounded-lg bg-black/30 backdrop-blur-sm flex justify-between items-center"> {/* Changed justify-center to justify-between */}
         <div className="flex items-center justify-center flex-grow"> {/* Allow this div to grow */}
        ...
        <Card className="bg-white/90 dark:bg-slate-900/90 text-foreground shadow-xl rounded-xl backdrop-blur-md border border-white/30">
          <CardHeader>
            ...
            <CardDescription className="text-muted-foreground text-sm">
               ...
               <div className="text-foreground">Expected Format: <code className="bg-black/20 dark:bg-white/20 px-1 py-0.5 rounded text-xs">Date,Location,Water_Temperature_C,Salinity_PSU,pH_Level,Dissolved_Oxygen_mg_L,Turbidity_NTU,Nitrate_mg_L</code></div>
             </CardDescription>
          </CardHeader>
          <CardContent>
             {/* New Input Fields */}
            ...
            <Textarea
              ...
              className={cn(
                ...
                isDragging && "border-cyan-500 bg-cyan-50 dark:bg-cyan-900/30 border-dashed border-2" // Style for drag-over state
              )}
            />
            <Button
              onClick={analyzeData}
            ...
            {isLoading && (
              <div className="w-full px-4 mt-4">
                <Progress value={analysisProgress} className="w-full [&>div]:bg-cyan-400 h-2.5 rounded-full bg-white/30 dark:bg-black/30" />
                ...
                <Table className="min-w-full">
                  <TableHeader className="bg-cyan-600/10 dark:bg-cyan-400/10">
                    <TableRow className="border-b border-cyan-200/30 dark:border-cyan-700/30">
                      ...
                      {/* Responsive Headers: Use abbreviations or icons on small screens */}
                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground hidden sm:table-cell">Water Temp (°C)</TableHead>
                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground sm:hidden" title="Water Temperature (°C)"><Thermometer className="w-4 h-4 inline-block" /></TableHead>

                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground hidden sm:table-cell">Salinity (PSU)</TableHead>
                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground sm:hidden" title="Salinity (PSU)"><Waves className="w-4 h-4 inline-block" /></TableHead>

                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground hidden sm:table-cell">pH Level</TableHead>
                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground sm:hidden" title="pH Level"><Beaker className="w-4 h-4 inline-block" /></TableHead>

                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground hidden sm:table-cell">Oxygen (mg/L)</TableHead>
                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground sm:hidden" title="Dissolved Oxygen (mg/L)"><Wind className="w-4 h-4 inline-block" /></TableHead>

                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground hidden sm:table-cell">Turbidity (NTU)</TableHead>
                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground sm:hidden" title="Turbidity (NTU)"><CloudFog className="w-4 h-4 inline-block" /></TableHead>

                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground hidden sm:table-cell">Nitrate (mg/L)</TableHead>
                       <TableHead className="text-right font-medium py-3 px-4 border-r border-cyan-200/30 dark:border-cyan-700/30 text-foreground sm:hidden" title="Nitrate (mg/L)"><Droplet className="w-4 h-4 inline-block" /></TableHead>
                  ...
                  <AccordionContent data-graph-content className="p-4 border-t border-cyan-200/30 dark:border-cyan-700/30">
                    ...
                    <ChartContainer config={chartConfig} className="aspect-video h-[300px] w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <ScatterChart // Changed to ScatterChart for individual points
                        ...
                        >
                          ...
                          />
                           ...
                           {/* Scatter plot for suitability index */}
                            <Scatter
                                key={`suitabilityIndex-scatter`}
                                dataKey='suitabilityIndex'
                                fill={chartConfig.suitabilityIndex?.color || 'hsl(var(--primary))'}
                                shape="circle" // Use circles for points
                           ...
                           <Line
                             ...
                             dot={false} // Hide default dots from Line, Scatter handles them
                             activeDot={false}
                             name={chartConfig.suitabilityIndex?.label || "Suitability Index"}
              ...
              <AccordionItem value={parameter.key} key={parameter.key} className="border-none">
                <Card className="bg-white/90 dark:bg-slate-900/90 text-foreground shadow-xl rounded-xl backdrop-blur-md border border-white/30 overflow-hidden">
                  ...
                  <AccordionContent data-graph-content className="p-4 border-t border-cyan-200/30 dark:border-cyan-700/30">
                    ...
                    <ChartContainer config={chartConfig} className="aspect-video h-[300px] w-full">
                       <ResponsiveContainer width="100%" height="100%">
                           <ScatterChart // Use ScatterChart to show individual points clearly
                           ...
                           >
                               ...
                               <RechartsLegend content={ <ChartLegendContent
                                     payload={
                                         ...
                                         .map(([key, config]) => ({
                                             value: config.label,
                                             type: key === 'prediction' ? 'dashed' : 'line', // Visual cue in legend
                                             id: key,
                                ...
                                {/* Scatter for actual data points */}
                                <Scatter
                                    key={`${parameter.key}-actual-scatter`}
                                    dataKey={(payload: AnalysisResult) => payload.isPrediction ? null : payload[parameter.key as keyof SensorData]}
                                    fill={chartConfig[parameter.key]?.color || 'black'} // Use parameter color or black for dots
                                    shape="circle"
                                    name={chartConfig[parameter.key]?.label || parameter.name}
                                />
                                {/* Line connecting actual data points */}
                                <Line
                                    key={`${parameter.key}-actual-line`}
                                    dataKey={(payload: AnalysisResult) => payload.isPrediction ? null : payload[parameter.key as keyof SensorData]}
                                    type="linear"
                                    stroke="black" // Black connecting line
                                    strokeWidth={1.5}
                                    dot={false} // Hide default dots from Line
                                    activeDot={false}
                                    isAnimationActive={false}
                                    connectNulls={false}
                                />

                               {/* Scatter for predicted data points */}
                               <Scatter
                                   key={`${parameter.key}-prediction-scatter`}
                                   dataKey={(payload: AnalysisResult, index: number) => {
                                       ...
                                       if (payload.isPrediction || (firstPredictionIndex !== -1 && index === firstPredictionIndex - 1)) {
                                           // Include the last actual point to connect the line
                                           return payload[parameter.key as keyof SensorData];
                                       }
                                       return null;
                                   }}
                                   fill={chartConfig.prediction.color} // Prediction dot color
                                   shape="circle"
                                   name={`${chartConfig[parameter.key]?.label || parameter.name} (Pred.)`}
                               />

                               {/* Dashed line connecting prediction points (and the last actual point) */}
                               <Line
                                   key={`${parameter.key}-prediction-line`}
                                   dataKey={(payload: AnalysisResult, index: number) => {
                                       const firstPredictionIndex = analysisResults.findIndex(d => d.isPrediction === true);
                                        if (payload.isPrediction || (firstPredictionIndex !== -1 && index === firstPredictionIndex - 1)) {
                                           // This dataKey makes the line connect the last actual point and all prediction points
                                           return payload[parameter.key as keyof SensorData];
                                       }
                                       return null;
                                   }}
                                   type="linear"
                                   stroke="black" // Keep connecting line black
                                   strokeWidth={1.5}
                                   strokeDasharray="5 5" // Dashed line for predictions
                                   dot={false} // Hide default dots from Line
                                   activeDot={false}
                                   isAnimationActive={false}
                                   connectNulls={true} // Connect across nulls to draw the prediction line
                               />
                           </ScatterChart>
                       </ResponsiveContainer>
                    </ChartContainer>
             ...
             <AccordionItem value="distribution" key="distribution" className="border-none">
                 <Card className="bg-white/90 dark:bg-slate-900/90 text-foreground shadow-xl rounded-xl backdrop-blur-md border border-white/30 overflow-hidden">
                   ...
                   <AccordionContent data-graph-content className="p-4 border-t border-cyan-200/30 dark:border-cyan-700/30">
                     ...
                     <ChartContainer config={chartConfig} className="aspect-video h-[300px] w-full">
                       {/* Data preparation and rendering logic remains the same */}
                       {(() => {
                           const actualData = analysisResults.filter(d => !d.isPrediction);
                           if (actualData.length === 0) return <p className="text-center text-muted-foreground">No data to display averages.</p>; // Handle no data case
                           ...
                           const averages = parameters.map(param => {
                               ...
                               const avg = sum / actualData.length;
                               return {
                           ...
                           // Determine Y-axis domain based on average values
                           const values = averages.map(a => a.value);
                           const yDomain: [number, number] = [
                             Math.min(0, Math.min(...values) * 0.9), // Start slightly below min or 0
                             Math.max(...values) * 1.1 // Go slightly above max
                           ];


                           return (
                               <ResponsiveContainer width="100%" height="100%">
                                    {/* Changed back to LineChart frame but kept Bar logic as requested */}
                                    {/* This setup is less standard for bar charts */}
                                   <LineChart // Using LineChart frame for vertical bars might require tweaks
                                       data={averages}
                                       layout="vertical" // Key for vertical bars
                                       margin={{ top: 5, right: 30, left: 20, bottom: 5 }}
                                   >
                                       <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border) / 0.5)" horizontal={false}/>
                                       {/* XAxis is now the value axis */}
                                       <XAxis type="number" domain={yDomain} stroke="hsl(var(--foreground))" tick={{ fontSize: 12, fill: 'hsl(var(--foreground))' }} axisLine={false} tickLine={false} />
                                       {/* YAxis is now the category axis */}
                                       <YAxis dataKey="name" type="category" stroke="hsl(var(--foreground))" tick={{ fontSize: 10, fill: 'hsl(var(--foreground))' }} axisLine={false} tickLine={false} width={150} interval={0} />
                                       <RechartsTooltip
                                           content={
                                               <ChartTooltipContent
                                                   labelClassName="text-sm font-medium text-foreground"
                                       ...
                                       />
                                        {/* Use Line component configured to look like Bars - less ideal */}
                                        {/* A proper <BarChart> with <Bar> component is recommended */}
                                       <Line
                                            dataKey="value"
                                            type="monotone" // Or linear
                                            stroke="transparent" // Hide the connecting line
                                            strokeWidth={0}
                                            isAnimationActive={false}
                                            // Custom dot rendering to simulate bars
                                            dot={(props) => {
                                                const { cx, cy, payload, value } = props;
                                                // This is a simplified representation; a true Bar component is better
                                                // Calculate bar width and position - this is tricky in LineChart
                                                const barHeight = 10; // Example fixed height
                                                return (
                                                    <rect
                                                        x={0} // Should ideally start from axis
                                                        y={cy - barHeight / 2}
                                                        width={cx} // Width is based on X value (cx)
                                                        height={barHeight}
                                                        fill={payload.fill}
                                                        radius={2}
                                                    />
                                        ...
                                        >
                                            {/* Recharts doesn't easily support individual cell colors in LineChart */}
                                        </Line>
